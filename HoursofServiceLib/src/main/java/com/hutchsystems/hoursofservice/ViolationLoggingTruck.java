...
import com.hutchsystems.hoursofservice.Repository.DurationLoggingTruck;
...
public class ViolationLoggingTruck extends ViolationModel {
    ...
    DurationLoggingTruck objCanada;
    int currentRule = 1;
    //   #endregion
    ...
    public ViolationLoggingTruck(Date fromDate, Date toDate, ArrayList<DutyStatus> list, ArrayList<DutyStatus> ruleList, int futureTime, int currentRule) {
        ...
        objCanada = new DurationLoggingTruck();
        objCanada.canadaFg = true;
        objCanada.OnDutyList.add(0d);
        objCanada.DrivingList.add(0d);
...
/// Created By: Deepak sharma
    // Created Date: 06 November 2018
    // Purpose: get object to get hours as per rule
    public DurationLoggingTruck HoursGet() {
        return objCanada;
    }
    ...
    public ArrayList<ViolationModel> Calculate() {
                ...
                for (int j = 0; j <= diff; j++) {
                    ...
                    if (j < diff) {
                        ...
                        if (!item.isFutureFg()) {
                            ...
                            if (objCanada.OnDutyList.size() == 7)
                            ...
                            if (objCanada.DrivingList.size() == 7)
                                objCanada.DrivingList.remove(0);
        ...
        Collections.sort(_violationList, ViolationLoggingTruck.dateAsc);
    ...
    private void ProcessViolations(DutyStatus item, double duration) {
       ...
       /* if (item.getRule() <= 2) // These are the rule when driver drives in Canada
        ...
        // check and add violation belongs to 37(1)(a)(i) section
        AddViolation("37.15(1)(a)(i)", item.getEventDateTime(), duration, objCanada.workShiftDriving);

        // check and add violation belongs to 37(1)(a)(ii) section
        AddViolation("37.15(1)(a)(ii)", item.getEventDateTime(), duration, objCanada.workShiftOnDuty);

        // check and add violation belongs to 37(1)(b)(i) section
        AddViolation("37.15(1)(b)(i)", item.getEventDateTime(), duration, objCanada.totalWorkshift);


        // check and add violation belongs to 37(2)(a) section
        AddViolation("37.15(2)(a)", item.getEventDateTime(), duration, objCanada.getRuleDriving());


        // check and add violation belongs to 37(2)(b) section
        AddViolation("37.15(2)(b)", item.getEventDateTime(), duration, objCanada.getRuleOnDuty());
        ...
        if (TimeUtility.getDiff(objCanada.LastBreak24Hours, item.getEventDateTime(), TimeUtility.Unit.DAY) > 7) {

            // check and add violation belongs to rule 25
            AddViolation("37.15(1)(b)(ii)", item.getEventDateTime());
        }
    }
    ...
    public void AddViolation(String rule, Date violationDate) {
        ...
        if (ruleId == 7 && !(violationInfo.getViolationDate().before(fromDate))) {
